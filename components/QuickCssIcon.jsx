...
module.exports = React.memo((compProps) => {
  ...
  if (compProps.inSetting) {
    ...
    return <Tooltip text={Messages.CSS_TOGGLER_GO_TO_QUICK_CSS_TOOLTIP} position='right'>
      {(props) => <svg viewBox='0 0 24 24' {...props} {...compProps}>
  ...
  let { main } = compProps;
  delete compProps.main;

  const handleOnContext = async () => {
    settingsModule.open(main.entityID)
  }
  ...
  return (
    <HeaderBar.Icon
      ...
      className="quickCSSIcon-tooltip-wrap"
      aria-label={Messages.CSS_TOGGLER_GO_TO_QUICK_CSS_OR_TOGGLER}
      tooltip={Messages.CSS_TOGGLER_GO_TO_QUICK_CSS_OR_TOGGLER}
    />
  );
});
